...
from networks.FAFN import FAFN
...
# gai 12.21
class MixFAFN(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.fr_fn = FAFN(dim=c1, hidden_dim=c2, act_layer=nn.GELU)
...
class MLP_FAFN(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.fr_fn = FAFN(dim=c1, hidden_dim=c2, act_layer=nn.GELU, use_eca=False)  # gai FFN→FRFN

    def forward(self, x):
        return self.fr_fn(x)


class MixD_FAFN(nn.Module):
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio=1, token_mlp="mix"):
        ...
        if token_mlp == "mix":
            self.mlp = MixFAFN(dim, int(dim * 4))
        elif token_mlp == "mix_skip":
            self.mlp = MixFAFN_skip(dim, int(dim * 4))
        else:
            self.mlp = MLP_FAFN(dim, int(dim * 4))
...
class FuseTransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio=1, fuse_mode="add"):
        ...
        self.attn = EfficientSelfAtten(dim, head, reduction_ratio)
        self.norm2 = nn.LayerNorm(dim)
        self.mlp = MixD_FAFN(dim, int(dim * 4), fuse_mode)
