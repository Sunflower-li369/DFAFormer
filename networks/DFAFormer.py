...
from networks.DIFF_Transformer import DIFFTransformer
...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(self, in_dim, key_dim, value_dim, height, width, head_count=1, token_mlp="mix"):
        ...
        if token_mlp == "mix":
            self.mlp = MixFAFN((in_dim * 2), int(in_dim * 4))
        elif token_mlp == "mix_skip":
            self.mlp = MixFAFN_skip((in_dim * 2), int(in_dim * 4))
        else:
            self.mlp = MLP_FAFN((in_dim * 2), int(in_dim * 4))
...
class EAFAB(nn.Module):   # efficient attention feature aggregation
    """
    input  -> x:[B, D, H, W]
    output ->   [B, D, H, W]
...
class CAFAB(nn.Module):   # channel attention feature aggregation
    """
    Input -> x: [B, N, C]
    Output -> [B, N, C]
    """
...
class DFATB(nn.Module):   # Dual Feature Aggregation Transformer Block
    """
    ...
    def __init__(self, in_dim, key_dim, value_dim, head_count=1, token_mlp="mix"):
        super().__init__()
        self.norm1 = nn.LayerNorm(in_dim)
        self.attn = EAFAB(in_channels=in_dim, key_channels=key_dim, value_channels=value_dim, head_count=1)
        self.norm2 = nn.LayerNorm(in_dim)
        self.norm3 = nn.LayerNorm(in_dim)
        self.channel_attn = CAFAB(in_dim)
        self.norm4 = nn.LayerNorm(in_dim)
        if token_mlp == "mix":
            self.mlp1 = MixFAFN(in_dim, int(in_dim * 4))
            self.mlp2 = MixFAFN(in_dim, int(in_dim * 4))
        elif token_mlp == "mix_skip":
            self.mlp1 = MixFAFN_skip(in_dim, int(in_dim * 4))
            self.mlp2 = MixFAFN_skip(in_dim, int(in_dim * 4))
        else:
            self.mlp1 = MLP_FAFN(in_dim, int(in_dim * 4))
            self.mlp2 = MLP_FAFN(in_dim, int(in_dim * 4))
...
class MiT(nn.Module):
    def __init__(self, image_size, in_dim, key_dim, value_dim, layers, head_count=1, token_mlp="mix_skip"):
        ...
        self.block1 = nn.ModuleList(
            [DFATB(in_dim[0], key_dim[0], value_dim[0], head_count, token_mlp) for _ in range(layers[0])]
        )
        self.norm1 = nn.LayerNorm(in_dim[0])

        self.block2 = nn.ModuleList(
            [DFATB(in_dim[1], key_dim[1], value_dim[1], head_count, token_mlp) for _ in range(layers[1])]
        )
        self.norm2 = nn.LayerNorm(in_dim[1])

        self.block3 = nn.ModuleList(
            [DFATB(in_dim[2], key_dim[2], value_dim[2], head_count, token_mlp) for _ in range(layers[2])]
        )
...
class MyDecoderLayer(nn.Module):
    def __init__(
        ...
        self.layer_former_1 = DFATB(out_dim, key_dim, value_dim, head_count, token_mlp_mode)
        self.layer_former_2 = DFATB(out_dim, key_dim, value_dim, head_count, token_mlp_mode)
