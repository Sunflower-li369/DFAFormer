...
from networks.SGFN import SGFN    # gai 1.21
...
class MixSGFN(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.fr_fn = SGFN(dim=c1, hidden_dim=c2, act_layer=nn.GELU)
...
class MixSGFN_skip(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.fr_fn = SGFN(dim=c1, hidden_dim=c2, act_layer=nn.GELU)
        self.norm1 = nn.LayerNorm(c1)  # gai 12.23: 确保norm1的输入维度与fr_fn的输出维度一致
        self.norm2 = nn.LayerNorm(c1)  # gai 12.23: 确保norm2的输入维度与fr_fn的输出维度一致
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        ...
        out = self.fr_fn(x)
        out = self.norm1(out)
        out = self.norm2(out + identity)
        return out


class MLP_SGFN(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        self.fr_fn = SGFN(dim=c1, hidden_dim=c2, act_layer=nn.GELU, use_eca=False)  # gai FFN→FRFN

    def forward(self, x):
        return self.fr_fn(x)


class MixD_SGFN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.fuse_mode == "add":
            ...
            fuse = torch.cat([ax, x], dim=-1)

        # If fuse_mode is "concat", we need to adjust the output dimension
        if self.fuse_mode != "add":
            fuse = self.fc2(fuse)
...
class OverlapPatchEmbeddings(nn.Module):
    ...
    输入: [B, C, H, W]
    输出: (patch_embeddings, H_out, W_out)
    """

    def __init__(self, patch_size: int = 7, stride: int = 4, padding: int = 1,
                 in_ch: int = 1, dim: int = 128):  # 医学图像是单通道
        super().__init__()
        self.proj = nn.Conv2d(
            in_channels=in_ch,
            out_channels=dim,
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, int, int]:
        ...
        px = self.proj(x)  # [B, dim, H_out, W_out]
        _, _, H_out, W_out = px.shape

        fx = px.flatten(2)  # [B, dim, H_out*W_out]
        fx = fx.transpose(1, 2)  # [B, H_out*W_out, dim]
        nfx = self.norm(fx)

        return nfx, H_out, W_out
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio=1, token_mlp="mix"):
        ...
        if token_mlp == "mix":
            self.mlp = MixSGFN(dim, int(dim * 4))
        elif token_mlp == "mix_skip":
            self.mlp = MixSGFN_skip(dim, int(dim * 4))
        else:
            self.mlp = MLP_SGFN(dim, int(dim * 4))
...
class FuseTransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio=1, fuse_mode="add"):
        ...
        self.attn = EfficientSelfAtten(dim, head, reduction_ratio)
        self.norm2 = nn.LayerNorm(dim)
        self.mlp = MixD_SGFN(dim, int(dim * 4), fuse_mode)
